...
- `In Implementation Review` で `PASS` を保存した直後に受けた有効なクローズ指示（`クローズ処理してください` または `クローズ処理`）は、このSkillが検証済みの対象変更に限り、`git add`、`git commit`、外部リモートのdefaultブランチである `origin/main` への通常のfast-forward `git push`、および対象Issueの `Done` 更新を、追加の会話上の承認なしに実行する明示的なユーザー承認として扱います。`git-add-commit-push`へ委譲するときは、対象Issue、対象ファイル、送信先 `origin/main`、通常のfast-forward pushであること、ならびに承認根拠が有効なクローズ指示であることを委譲入力に明記します。これはsandbox、GitHub/remote認証、ツールの外部送信安全審査を迂回する権限ではなく、それらが要求する承認・認証・安全確認を満たせない場合は停止します。`origin/main`以外の宛先、force push、対象外変更、別Issue、その他の安全停止条件には適用しません
