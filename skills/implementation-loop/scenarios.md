...
- `case_name` は共通カタログの正式な3値（`user_correction`、`external_operation_failure`、`workflow_contract_violation`）との完全一致だけを候補として扱います。Issue固有Planで候補を再定義せず、未知の名前やカタログ外の事象はCase化しません。Sourceはadd-case境界で `Workflow` へ正規化します
...
共通カタログ外の未知シグナル名、またはカタログ条件に一致しない事象も同じ境界で確認します。

期待結果：

- Payloadを作成せず、`add-case` 呼出しは0回です
- 推測でCase化せず、Case、Policy、Feedback Countを変更せずに通常のClose処理を継続します

## I3 候補あり・logical payload

共通カタログの3シグナル（`user_correction`、`external_operation_failure`、`workflow_contract_violation`）それぞれについて、Subject、Summary、Occurred At、Contextの根拠を取得できる事象を用意します。`user_correction` はユーザーがworkflowの行動・ルール修正を明示した事象、`external_operation_failure` は外部書込みの失敗・結果不明・誤対象・意図しない副作用の事象、`workflow_contract_violation` は未修正状態の修正済み報告・誤ったStatus遷移・必須ゲートのスキップ・未承認scope変更の事象として確認します。複数シグナルに見える同一発生は、優先順位を付けず判定不能として別に確認します。
...
- 同一発生が複数シグナルに見える場合は単一一致とせず、payload作成と `add-case` 呼出しを0回にします

## I4 Reviewer指摘の採用境界

Canonical Reviewerが必須指摘を返した場合について、(a)親Agentがユーザー要件・canonical Planと不整合または過剰と判断して採用しない場合、(b)整合を確認して採用し、実際のPlanまたは実装変更に至った場合を確認します。Reviewerの任意提案は別に確認します。初期カタログにReviewer系シグナルは含めません。

期待結果：

- (a)および任意提案では、Reviewerの指摘だけを理由にCaseを作成しません
- (b)でもReviewerの指摘・採用・修正そのものはCase候補にしません。採用された変更が別途 `workflow_contract_violation` の条件に明確に一致する場合だけ、そのworkflow事実を候補にします
...
- `external_operation_failure`（外部書込みの失敗、結果不明、誤対象、意図しない副作用）の場合だけ、必須証拠に基づくpayloadを作成します。通常の人間確認待ち、通常のTest失敗、内部のphase停止はCase候補にしません
...
- 単一シグナルへの一致自体が不明確な場合はBLOCKEDではなく判定不能として、payloadと `add-case` 呼出しを0回のまま通常Closeを継続します
