...
4. canonical Plan本文から `plan-fingerprint-v1` のSHA-256を算出し、Issue ID、mode、profile、Test判定、`blockedBy` snapshotとともにPlan Review packetへ渡す。`blockedBy` snapshotは今回のPlanが依存する現在の`blockedBy`のIssue IDを昇順で格納した`relations_snapshot` JSON objectとする。`blocks` / `relatedTo`はこのmetadataに含めない
...
Planning保存後はIssue、Description、Status、Labels、Planが依存する`blockedBy`、Commentsを再取得し、保存済みcanonical Plan、`plan_fingerprint`、mode/profile、`test_decision`、`blockedBy` snapshotをReviewerへ渡して同一実行でPlan Reviewへ進みます。Reviewerはこれら二つのmetadataを変更せず返し、再取得値が保存前の意図と一致しない場合はBLOCKEDです。`relatedTo`／`blocks`の変更だけではBLOCKEDにしません。
...
Reviewerは要求適合、Repository整合、受入条件、検証可能性、未確認事項、Plan fingerprint、mode/profile、Test判定、`blockedBy` snapshotと、PlanがIssue達成に必要な最小scopeであることを確認します。`relatedTo`／`blocks`はscope・受入条件への実質影響がある場合だけ確認対象にします。
