...
5. 通常IssueでHuman AcceptanceがPASSの場合、candidate SHAが現在の対象HEADと一致し、対象pathにAcceptance後の未コミット変更がないことを確認する。Close先remote/refを確定し、Linearへ記録・readback済みの当該Issue checkpoint chainについて、そのtarget refからのlive reachabilityを確認する。target refから到達不能なcheckpointだけを古い順に並べ、今回の通常pushで公開を許可する `allowed_checkpoint_shas` とする。別remote/refへ先行push済みでもClose先target refから未到達なら含め、target refから既に到達可能なら含めない。target candidate SHA、送信先とともに後続Git段階の `publish-checkpoint` へ渡し、target refからHEADまでのoutgoing commit chain全体が許可列と完全一致する場合だけ既存candidate chainをpushする。新しいcommitは作成しない。Candidateが今回のtarget refから既に到達可能ならremote同期だけを確認する。outgoing chainに対象Issue外・由来不明・未承認commitが含まれる、許可列に不足・余剰・順序不整合がある、候補SHA不一致、未確認差分、remote先行/分岐、push失敗・結果不明では `Done` に進めない。Human AcceptanceがFAILならcandidateを保持して明示的な再開境界へ戻し、pushやDone化を行わない
...
9. `add-case`成功後、対象scopeをRepository単位に分け、各Repositoryごとに `git-add-commit-push` へ対象範囲とクローズ指示を渡して委譲する。通常Issueは前項のtarget candidate SHA、target remote/ref、target ref基準の `allowed_checkpoint_shas` を渡した `publish-checkpoint`、Spikeまたはcandidateを持たない公開は既存の公開契約に従う。Policy生成・Relation設定・Feedback Count加算・Review完了はこの振り返りで行わない
...
Git操作の安全条件、staging、commit、remote選択、target ref reachability、outgoing provenance、push、push後検証は `git-add-commit-push` をSource of Truthとします。
