...
3. 通常Issueは、Statusが `In Implementation Review` で保存された最新のImplementation完了・検証記録とHuman Acceptance確認点に加え、`candidate_commit`、push状態、対象SHAを確認する。AIの独立ReviewのPASSを前提にしない。Spikeは `In Implementation Review` の最新Result Reviewが `DECISION_READY` で、対象・証拠・判断基準に意味のある変更がないことを確認する
4. 現在の依頼内に明示的なClose指示があることを確認する。Reviewの正判定だけで `Done` へ進めない
5. 通常IssueでHuman AcceptanceがPASSの場合、candidate SHAが現在の対象HEADと一致し、対象pathにAcceptance後の未コミット変更がないことを確認する。未pushなら、Linearへ記録・readback済みの当該Issue checkpointのうち現在remoteへ未送信と記録されたSHAを古い順に並べ、今回の通常pushで公開を許可する `allowed_checkpoint_shas` とする。target candidate SHA、送信先とともに後続Git段階の `publish-checkpoint` へ渡し、remoteからHEADまでのoutgoing commit chain全体が許可列と完全一致する場合だけ既存candidate chainをpushする。新しいcommitは作成しない。既にcandidateがpush済みならremote同期だけを確認する。outgoing chainに対象Issue外・由来不明・未承認commitが含まれる、許可列に不足・余剰・順序不整合がある、候補SHA不一致、未確認差分、remote先行/分岐、push失敗・結果不明では `Done` に進めない。Human AcceptanceがFAILならcandidateを保持して明示的な再開境界へ戻し、pushやDone化を行わない
...
9. `add-case`成功後、対象scopeをRepository単位に分け、各Repositoryごとに `git-add-commit-push` へ対象範囲とクローズ指示を渡して委譲する。通常Issueは前項のtarget candidate SHAと `allowed_checkpoint_shas` を渡した `publish-checkpoint`、Spikeまたはcandidateを持たない公開は既存の公開契約に従う。Policy生成・Relation設定・Feedback Count加算・Review完了はこの振り返りで行わない
10. 全RepositoryでGit Skillが成功、または送信すべき変更なしを確認できた場合だけ `Done` へ更新する
11. いずれかのCase処理・Git処理の失敗・結果不明・Issueまたは必要なReview/Acceptance記録の不一致ではStatusを維持する
12. `Done` 更新後に再取得確認する

Git操作の安全条件、staging、commit、remote選択、outgoing provenance、push、push後検証は `git-add-commit-push` をSource of Truthとします。
