...
5. 通常IssueでHuman AcceptanceがPASSの場合、candidate SHAが現在の対象HEADと一致し、対象pathにAcceptance後の未コミット変更がないことを確認する。未pushなら後続のGit段階で `publish-candidate` の対象とし、既存candidateをpushする。新しいcommitは作成しない。既にpush済みならremote同期だけを確認する。候補SHA不一致、未確認差分、push失敗・結果不明では `Done` に進めない。Human AcceptanceがFAILならcandidateを保持して明示的な再開境界へ戻し、pushやDone化を行わない
...
9. `add-case`成功後、対象scopeをRepository単位に分け、各Repositoryごとに `git-add-commit-push` へ対象範囲とクローズ指示を渡して委譲する。通常Issueは前項の `publish-candidate`、Spikeまたはcandidateを持たない公開は既存の公開契約に従う。Policy生成・Relation設定・Feedback Count加算・Review完了はこの振り返りで行わない
