...
def _keychain_credential() -> str | None:
    try:
        result = subprocess.run(KEYCHAIN_COMMAND, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=KEYCHAIN_TIMEOUT_SECONDS, check=False)
...
def review_text(
    ...
    *,
    ...
    if not safe or state.get("stopped") is True:
        return None
    api_key = _keychain_credential()
    if not api_key:
        return None
