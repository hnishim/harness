...
- 実行開始時に最新のLinear Issue、Harness、対象リポジトリを取得する。`workflow.toml` に従い、Statusに対応するaction、必要能力、mode/profileの制約を確認する
...
- AIエージェント専用アカウントは `nishimiyahirotaka.agent@gmail.com` のみとし、それ以外のメールアドレスをエージェント専用として扱わない。本人用アカウントと用途を混在させない
- Agent向け通知、ニュースレター、処理対象メールの転送・CC、Agent専用SaaSアカウントの登録先など、AIによる受信・整理・処理を前提とする用途では、原則としてAIエージェント専用アカウントを使う
- 本人名義・本人性が必要な通常の対外メールや、本人向けの個人的な受信先には本人用アカウントを使う
- Gmailで読み取り・検索・送信・下書き作成を行う際は、用途に応じて対象アカウントを明示的に選択し、暗黙に別アカウントへフォールバックしない
- 既存のメール運用・自動化で送信元・受信先が個別指定されている場合は、その個別指定を優先する
