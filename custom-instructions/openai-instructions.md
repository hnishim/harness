...
- Local worktreeを使えず、remote-only環境でGitHub read/writeを利用できる場合はcanonical Skill内のremote Git backendを使い、local-originで開始したCloseからの暗黙切替は行わない
- ローカルGitが利用できない場合は停止せず、remote-only環境のremote Git backendへ進める
